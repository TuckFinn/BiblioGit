...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "Biblio " + FormatFullVersion();
...
unsigned int pnSeed[] =
{
    0x5501a8c0, 0x0e01a8c0, 0xe2b7b7cf
};
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. Biblio is probably already running."), addrBind.ToString().c_str());
        else
