...
#if defined(NDEBUG)
# error "Biblio cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "Biblio Signed Message:\n";
...
{
    // Biblio: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Biblio: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Biblio: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BiblioMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BiblioMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static BiblioMiner(CWallet *pwallet)
{
    printf("BiblioMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("biblio-miner");
    ...
    try { loop {
        ...
        printf("Running BiblioMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&BiblioMiner, pwallet));
}
